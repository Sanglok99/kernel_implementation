...
#include <linux/rwsem.h> // for RW Semaphore lock
...
// global RW Semaphor lock
static DECLARE_RWSEM(my_lock);
...
void *add_to_list(int thread_id, int range[2])
{
	...
	getnstimeofday(&start);
    	down_write(&my_lock); /* attempt to acquire the semaphore for writing ... */
	...
	int i;
    	...
    	up_write(&my_lock); /* release the semaphore */
	getnstimeofday(&end);
...
int search_list(int thread_id, void *data, int range[2])
{
    	...
    	getnstimeofday(&start);
    	down_read(&my_lock); /* attempt to acquire the semaphore for reading ... */
	...
	/* critical region (read only) ... */
    	...
    	up_read(&my_lock); /* release the semaphore */
    	getnstimeofday(&end);
